...
4. **Cold Tier is Off-Limits**: NEVER read the `cold_index.md`. It is purely for archival purposes and excluded from the Vector DB.
...
### 3. `search_wiki.py`
Performs a Hybrid Semantic Search over the Wiki, using Semantic Chunking to prevent Context Bloat. Automatically skips Cold files.
Usage: `python path/to/skill/scripts/search_wiki.py --dir <wiki_dir> --query "<search text>" --top <N> --tags "<tag_name>"`
